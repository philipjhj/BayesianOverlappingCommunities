...
for t=1:length(logP_tmp)
    ...
    for i=1:no_integrations
                cumsum_P(i)=cumsum_P(i)*d1(i)^max_comp(i)+0.5*max_comp(i);
                %logQ_tmp(i,t)=log(cumsum_P(i))+max_new(i)+z(i); 
                cumsum_P(i)=cumsum_P(i)+0.5*max_comp(i);
                max_old(i)=max_new(i)*max_comp(i)+(1-max_comp(i))*max_old(i);
                q=d2(i);
                cumsum_P(i)=cumsum_P(i)+0.5*q*(1-max_comp(i));
                logQ_tmp(i,t)=log(cumsum_P(i))+max_old(i)*(1-max_comp(i))+max_new(i)*max_comp(i)+z(i);       
                cumsum_P(i)=cumsum_P(i)+0.5*q*(1-max_comp(i));
    end
end
